...
abstract class X86Compiler extends BugReporter with Codegen {
  ...
  var is_inside_fundef = 0
  var number_of_calls_inside_fundef = 0
  /*
  ...
  def trans(exp: Exp, sp: Loc)(env: LocationEnv): Unit = exp match {
    ...
    case Ref(x) =>
      env(x) match {
        case Reg(sp1) => emitln(s"movq ${loc(env(x))}, ${loc(sp)}")
        case Func(name) =>
          //println("ERRRRROROROROOR")
          //emitln(s"movq (${Func(name)}), ${loc(sp)}")
          //??? // Extra credit
      }
    ...
    case LetRec(funs, body) =>
      ...
      //println("=======LetRec=========")
      //println("funs:"+funs)
      //println("funsLoc:"+funsLoc)

      // TODO complete the code
      //var i = -1
      funs foreach {
        case cur_fun@FunDef(name, args, _, _) =>
          //i = i + 1
          var off_set = args.length
          trans(cur_fun,sp+off_set)(env.withVals(funsLoc))
      }
      emitln("#################################################\n\n", 0)
      emitln("###################### MAIN #####################", 0)
      //////////// DO NOT CHANGE////////////////
    ...
    case FunDef(fname, args, _, fbody) =>
      ...
      val argLoc = args map {
        case Arg(name, _, _) =>
          i = i + 1
          //println("Reg(i):"+loc(Reg(i)),i)
          (name, Reg(i))
      }
      is_inside_fundef = 1
      trans(fbody,sp)(env.withVals(argLoc))
      is_inside_fundef = 0
      number_of_calls_inside_fundef = 0
      emitln(s"movq ${loc(sp)},%rax")
      //////////// DO NOT CHANGE////////////////
      emitln("movq %rbp, %rsp\t# reset frame")
      emitln("popq %rbp")
    ...
    case App(fun, args) =>
      ...
      fun.tp match{
        case ArrayType(typ) =>
          emitln(s"movq $fLoc,%rax")
          emitln(s"movq (%rax,${loc(sp)},8),%rax")

        case _ =>
          if(is_inside_fundef==0) {
            var count = 0
            while(loc(sp) != loc(Reg(count))){
              emitln(s"push ${avRegs(count)}")
              count = count + 1
            }
            var k = -1
            args foreach {
              case cur_arg =>
                k = k + 1
                emitln(s"movq ${loc(sp + k)},${avRegs(k)}")
            }
            emitln(s"call $fLoc")
            while(count > 0){
              count = count - 1
              emitln(s"pop ${avRegs(count)}")
            }
            emitln(s"movq %rax,${loc(sp)}")
            /*
            if (num_of_calls == -1) {
              ...
              var count = 0
              for (count <- 0 until num_of_calls) {
                emitln(s"push ${avRegs(count)}")
              }
              var k = -1
              args foreach {
              ...
              count = 0
              for (count <- 0 until num_of_calls) {
                emitln(s"pop ${avRegs(num_of_calls - 1 - count)}")
              }
              emitln(s"movq %rax,${loc(num_of_calls)}")
            ...
            */
            ...
            emitln(s"movq %rax,${loc(sp)}")
            number_of_calls_inside_fundef = number_of_calls_inside_fundef + 1
          }
      }
     /*
    ...
    case ArrayDec(size, _) =>
      ...
      // TODO
      trans(size,sp)(env)
      emitln(s"movq ${loc(sp)},-8(%rsp)")
      //emitln(s"push ${loc(sp)}")
      //emitln(s"call malloc")
