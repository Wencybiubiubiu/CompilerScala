...
class ValueInterpreter extends Interpreter with BugReporter {
  ...
  def evalTer(op: String)(v: Val, w: Val, z: Val) = (op, v, w, z) match {
    ...
    case _ =>
      //println(op)
      //println("++++++++")
      BUG(s"ternary operator $op undefined")
  }
  ...
  def eval(exp: Exp)(env: ValueEnv): Val = exp match {
    ...
    case LetRec(funs, body) =>
      ...
      funcs foreach { case (_, func@Func(_, _, _)) =>
        //println(func)
        func.withVals(funcs)
      }
      //println(funcs)
...
class StackInterpreter extends Interpreter with BugReporter {
  ...
  def evalBin(op: String)(sp: Loc, sp1: Loc) = (op, memory(sp), memory(sp1)) match {
    case ("+",  Cst(x: Int), Cst(y: Int)) =>
      memory(sp) = Cst(x + y)
      //println("PRim:"+memory(sp))
    ...
    case ("<" , Cst(x: Int), Cst(y: Int)) => memory(sp) = Cst(x <  y)
    case (">" , Cst(x: Int), Cst(y: Int)) => memory(sp) = Cst(x >  y)
    case ("block-get", Cst(arr: Array[Any]), Cst(i: Int)) => //TODO
      //if (arr(i) == null)
      //  BUG(s"uninitialized memory")
      memory(sp) = Cst(arr(i))
    case _    => BUG(s"Binary operator $op undefined")
  ...
  def evalPrim(op: String)(idxs: List[Int]) = idxs match {
    ...
    case List(sp, sp1)      =>
      //println("PRIMMM:"+List(memory(sp), memory(sp1)) )
      evalBin(op)(sp, sp1)
  ...
  def eval(exp: Exp, sp: Loc)(env: LocationEnv): Unit = exp match {
    ...
    case FunDef(_, args, _, fbody) =>
      //TODO
      //println("FunDef")
      //val temp = Func(args map { arg => arg.name }, fbody, env)
    ...
    case LetRec(funs, body) =>
      ...
      //var i = 0;
      val funcs = funs map {
        case fun@FunDef(name, _, _, _) =>
          //memory(sp) = Cst(eval(fun,sp)(env))
          //sp = sp + 1
          i = i+1
          eval(fun,sp+i)(env)
          println("funcs:" + memory(sp+i))
          (name,memory(sp+i),sp+i)

      }
      println(funcs)
      val temp = funcs map {
        case (name, func@Func(args, fbody, envs),spnum) =>
          println("+++++")
          (name,spnum)
      }
      ...
      // Add all functions to the functions environment (recursion)
      //println("temp:"+temp)
      //TODO: recursive does not finish
      funcs foreach { case (_, func@Func(args, fbody, envs),spnum) =>
          //println(func)
          //println(args)
          //println(fbody)
          //println("DEBUG")
      ...
      //println("DEBUG")
      i = i + 1
      eval(body,sp+i)(env.withVals(temp))
      memory(sp) = memory(sp+i)
      //println("body:" + memory(sp+i) + "," + (sp+i))
    case App(fun, args) =>
      //TODO
      // Evaluate the arguments
      //println("App:")
      ...
      i = i + 1
      eval(fun,sp+i)(env)
      //println("fun:" + memory(sp+i) + "," + (sp+i))
      memory(sp+i) match {
        case Func(fargs, fbody, fenv) =>
          i = i + 1
          eval(fbody,sp+i)(fenv.withVals(fargs zip eargs))
          //println("FUNC case:" + memory(sp+i))
        case Primitive("getchar") =>
          Cst(Console.in.read)
        case Primitive("putchar") =>
          //i = i + 1
          val List(Cst(c: Int)) = memory(sp+i)
          Console.out.write(c)
          Console.out.flush
          Cst(())
      }
      //println("HHHH:"+memory(sp+i))
      memory(sp) = memory(sp+i)
    case ArrayDec(size, _) =>
      //TODO
      //println("ArrayDec")
      eval(size,sp)(env)
      memory(sp) = Cst(new Array[Any](sp))
      //println(memory(sp).toString)
  }
}
